...
#define BottomLineWidth 60

@interface JGTitleView () {
    CGFloat _BottomLineMarginX;
}
...
- (void)setupBottomLine{
    ...
    _BottomLineMarginX = (self.titleLabels.firstObject.bounds.size.width - BottomLineWidth) / 2.0f;
    
    self.bottomLine.frame = CGRectMake(_BottomLineMarginX, self.bounds.size.height - self.style.bottomLineH, BottomLineWidth, self.style.bottomLineH);
}
...
- (void)titleLabelClick:(UITapGestureRecognizer *)tap{
    ...
    if (self.style.isShowBottomLine){
        [UIView animateWithDuration:0.5 animations:^{
            self.bottomLine.frame = CGRectMake(currentLabel.frame.origin.x + _BottomLineMarginX, self.bottomLine.frame.origin.y, BottomLineWidth, self.bottomLine.frame.size.height);
        }];
    }
...
- (void)setTitleWithProgress:(CGFloat)progress sourceIndex:(NSInteger)sourceIndex targetIndex:(NSInteger)targetIndex{
    ...
    if (self.style.isShowBottomLine){
        ...
        CGFloat x = sourceLabel.frame.origin.x + moveTotalX * progress + _BottomLineMarginX;
        
//        CGFloat width = sourceLabel.frame.size.width + moveTotalW * progress;
        
        self.bottomLine.frame = CGRectMake(x, self.bottomLine.frame.origin.y,BottomLineWidth, self.bottomLine.frame.size.height);
    }
