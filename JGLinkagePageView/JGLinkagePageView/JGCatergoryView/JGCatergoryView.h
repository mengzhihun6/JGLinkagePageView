...
/**
 一、是什么：JGCatergoryView是一个轻量级的顶部分类视图控件，只需要通过简单的设置，你就可以快速集成该控件， 控件目前暂时有底部横条移动，椭圆背景移动，文字缩放，文字颜色变化，和文字颜色渐变五种效果，五种效果可以叠加使用也可以单一使用，具体效果请查看demo,
 */

/**
 二、如何使用：
 1、导入`JGCatergoryView.h`头文件
 
 2、如果是当前控制器是被导航控制器管理，也就是说上方有导航栏，必须对当前控制器做如下设置：`self.automaticallyAdjustsScrollViewInsets = NO;`否则控件显示会有问题
 
 3、初始化该控件，代码和stroyboard都可以，stroyboard的话，直接拖入一个View并修改Class为`JGCatergoryView`即可;
 
 4、设置数据源titles属性（必须）
 
 5、设置与该控件关联的ScrollView（必须）
 
 6、配置相关的属性即可使用，可自定义的属性比较多，请自行去`JGCatergoryView.h`中查看，更详请见地址中的demo
 */


#import <UIKit/UIKit.h>

@class JGCatergoryView;

@protocol JGCatergoryViewDelegate <NSObject>
...
@interface JGCatergoryView : UIView
...
/**代理，可监听item按钮点击*/
@property (nonatomic, assign) id<JGCatergoryViewDelegate> delegate;
